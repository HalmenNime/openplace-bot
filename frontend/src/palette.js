...
export const palette = sortIdxs.map((idx) => {
    return {
        idx: idx,
        name: wplacePalette[idx].name,
        r: wplacePalette[idx].rgb[0],
        g: wplacePalette[idx].rgb[1],
        b: wplacePalette[idx].rgb[2],
        a: idx == 0 ? 0 : 255,
        isPremium: idx >= 32,
    };
});
...
export function dithering(imageData, usePaidColors = false) {
    ...
    for (let y = 0; y < height; y++) {
        for (let x = 0; x < width; x++) {
            ...
            // Tính lỗi màu sắc
            const error = {
                r: currentColor.r - closestColor.r,
                g: currentColor.g - closestColor.g,
